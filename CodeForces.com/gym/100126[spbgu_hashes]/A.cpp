...
int main()
{
    ...
    if(sizeOfResult!=-1)
    {
        ...
        current.clear();
        for(int i = 0; i<A[0].size-sizeOfResult; i++)
            current.insert(A[0].getHash(i,i+sizeOfResult));

        for(int _i = 1; _i<n; _i++)
        {
            next.clear();
            for(int i = 0; i<A[_i].size-sizeOfResult; i++)
                next.insert(A[_i].getHash(i,i+sizeOfResult));
        ...
        for(int i = 0; i<A[0].size-l; i++)
        {
            if(current.count(A[0].getHash(i,i+l) ) )
            {
                cout << string(A[0].s.begin()+i+1, A[0].s.begin()+i+1+l+1);
                break;
            }
        }
    }
