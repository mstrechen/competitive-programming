...
struct  SegmentTreeSumm
{
    ...
    SegmentTreeSumm(int n)
    {
        ...
        while(_size<n)
            _size*=2;
        mins = vector<long long>(_size*2, INF);
    }
    ...
    SegmentTreeSumm operator + (SegmentTreeSumm a)
    {
        ...
        {
            a.mins[i] = sum(this->mins[i], a.mins[i]);
        }
        return a;
    }
    ...
    void build(int n)
    {
        ...
        {
            mins[i] = sum(mins[i*2],mins[i*2+1]);
        }
    }
    ...
    int _getMin(int l, int r, int minL, int maxR, int _i)
    {
        ...
        if(l==minL&&r==maxR)
            return mins[_i];
        int mid = (minL+maxR)/2;
        return  sum(_getMin(l, min(mid,r), minL, mid, _i*2),
...
struct  SegmentTreeSumm2d
{
    ...
    int _getMin(int l, int r, int minL, int maxR, int _i, int x, int y)
    {
        ...
        int mid = (minL+maxR)/2;
        return  sum(_getMin(l, min(mid,r), minL, mid, _i*2,x,y),
