...
struct Buckets
{
    ...
    vector<pair<HashPair, int> > memAndNext;

    int firstUnused{0};

    Buckets(int cnt, int capacity) : beginnings(cnt, -1), memAndNext(capacity, {HashPair(0,0), -1}){}
    ...
    {
        memAndNext[firstUnused].first = toPush;
        memAndNext[firstUnused].second = beginnings[bucket];
    ...
    struct Iterator : iterator<std::forward_iterator_tag, HashPair>
    {
        ...
        Iterator & operator ++ ()
        {
            pos = src.memAndNext[pos].second;
            return *this;
        }
        HashPair & operator *()
        {
            return src.memAndNext[pos].first;
        }
