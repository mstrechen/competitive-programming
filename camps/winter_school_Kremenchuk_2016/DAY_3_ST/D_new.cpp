...
struct PersistentSegmentTree
{
    struct Node
    {
        ...
        Node(){}
        Node(UInt val, Node * L, Node * R) : val( (val + getVal(L) + getVal(R))%MOD ), l(L), r(R) {}
    };

    struct MemoryManager
    {
        vector<Node> mem{vector<Node>(200*1000*64)};
        int be{0};
        Node * operator ()(UInt val = 0, Node * L = nullptr, Node * R  = nullptr)
        {
            mem[be] = Node(val, L, R);
            return &mem[be++];
        }
    ...
    PersistentSegmentTree(int x) : root(new_Node(UInt(x))){}
    ...
    Node * _set(Node * cur,long long pos, UInt val, long long minL, long long maxR)
    {
        ...
        long long mid = (minL+maxR)/2;
        if(cur == nullptr)
            return new_Node(0, _set(nullptr, pos, val, minL, mid),
                               _set(nullptr, pos, val, mid+1, maxR));
        return new_Node(0, _set(cur->l, pos, val, minL, mid),
